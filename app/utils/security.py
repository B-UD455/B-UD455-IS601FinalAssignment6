...
def generate_verification_token(length: int = 16) -> str:
    """
    Generates a secure verification token of a specified length.
    
    Args:
        length (int): The length of the token to generate (default is 16 bytes).

    Returns:
        str: The generated verification token.

    Raises:
        ValueError: If an invalid length is provided.
    """
    if length < 16:
