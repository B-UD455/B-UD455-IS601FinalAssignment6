...
# Existing user routes omitted for brevity...

@router.post("/users/{user_id}/upgrade", response_model=UserResponse, tags=["User Management Requires (Admin or Manager Roles)"], name="upgrade_user_to_professional")
async def upgrade_user_to_professional(user_id: UUID, request: Request, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
    ...
    Returns:
        The upgraded user's information along with links to related actions.
    """
    user = await UserService.get_by_id(db, user_id)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    ...
    updated_user = await UserService.update(db, user_id, user.model_dump(exclude_unset=True))
