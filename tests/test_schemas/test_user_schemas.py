...
# Parametrized tests for password validation (added)
@pytest.mark.parametrize("password", ["SecurePassword123!", "Password!123", "ValidPass#2024"])
def test_user_create_password_valid(password, user_create_data):
    user_create_data["password"] = password
    user = UserCreate(**user_create_data)
    assert user.password == password

@pytest.mark.parametrize("password", ["short", "123", "password", "password!@#"])
def test_user_create_password_invalid(password, user_create_data):
    user_create_data["password"] = password
    with pytest.raises(ValidationError):
        UserCreate(**user_create_data)
        
def test_user_profile_picture_url_schema_invalid():
    with pytest.raises(ValidationError):
        UserUpdate(profile_picture_url="ftp://invalid-url.com")        
