...
def normalize_url(url):
    ...
    encoded_query = urlencode(sorted_query_items, doseq=True)
    normalized_url = urlunparse(parsed_url._replace(query=encoded_query))
    return normalized_url.rstrip('/').split('#')[0]  # Strip fragment if present
...
@pytest.mark.asyncio
async def test_generate_pagination_links_edge_case(mock_request):
    skip = 0
    limit = 10
    total_items = 5
    links = generate_pagination_links(mock_request, skip, limit, total_items)
    # In this case, there should be no "next" link, and only the "self" link should be generated
    assert len(links) == 1  # Only the self link should be returned
    assert links[0].rel == "self"
    expected_self_url = "http://testserver/users?limit=10&skip=0"
    assert normalize_url(str(links[0].href)) == normalize_url(expected_self_url), "Self link should match expected URL"
