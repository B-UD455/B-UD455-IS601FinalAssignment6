...
def test_hash_password_with_edge_cases(password):
    """Test hashing various edge cases."""
    hashed = hash_password(password)
    assert isinstance(hashed, str) and hashed.startswith('$2b$'), "Should handle edge cases properly"

def test_verify_password_with_edge_cases():
    """Test verifying passwords with edge cases."""
    password = " "
    hashed = hash_password(password)
    assert verify_password(password, hashed) is True
...
def test_hash_password_with_invalid_rounds():
    """Test hashing with invalid rounds (negative or non-integer values)."""
    password = "secure_password"
    with pytest.raises(ValueError):
        hash_password(password, -1)  # Negative rounds should raise an error
    with pytest.raises(ValueError):
        hash_password(password, "not_a_number")  # Non-integer rounds should raise an error

def test_verify_empty_password():
    """Test verifying an empty password string."""
    password = ""
    hashed = hash_password(password)
    assert verify_password(password, hashed) is True
