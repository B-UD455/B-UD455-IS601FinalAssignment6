...
async def test_user_linkedin_url_update(db_session: AsyncSession, user: User):
    """
    Tests the LinkedIn URL update functionality.
    """
...
async def test_user_github_url_update(db_session: AsyncSession, user: User):
    """
    Tests the GitHub URL update functionality.
    """
    profile_github_url = "http://www.github.com/profile"
    user.github_profile_url = profile_github_url
    await db_session.commit()
    await db_session.refresh(user)
    assert user.github_profile_url == profile_github_url, "The GitHub URL did not update"
...
@pytest.mark.asyncio
async def test_is_professional_default(user: User):
    """
    Tests that the is_professional field defaults to False.
    """
    assert not user.is_professional, "Default value for is_professional should be False"

@pytest.mark.asyncio
async def test_update_is_professional(db_session: AsyncSession, user: User):
    """
    Tests updating the is_professional field.
    """
    user.is_professional = True
    await db_session.commit()
    await db_session.refresh(user)
    assert user.is_professional, "is_professional should update to True after modification"
'''
...
async def test_user_role(db_session: AsyncSession, user: User, admin_user: User, manager_user: User):
    ...
    # Ensure that 'manager_user' is correctly considered a professional if that's part of the feature
    assert not manager_user.is_professional, "Manager should not be a professional by default"
