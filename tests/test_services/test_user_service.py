...
from tests.test_schemas.test_user_schemas import user_create_data,user_base_data
...
'''
# Test invalid URL for profile picture
async def test_user_profile_picture_url_invalid(db_session, user):
    user.profile_picture_url = "ftp://invalid-url.com"
    with pytest.raises(ValidationError):
        await UserService.update(db_session, user.id, {"profile_picture_url": user.profile_picture_url})
'''
# Test invalid URL for profile picture
async def test_user_profile_picture_url_invalid(db_session, user, caplog):
    user.profile_picture_url = "ftp://invalid-url.com"
...
#AdditionalTests_____________________________________


    
def test_user_create_missing_email(user_create_data):
    user_create_data.pop("email")
    with pytest.raises(ValidationError):
        UserCreate(**user_create_data)

@pytest.mark.parametrize("weak_password", ["password", "PASSWORD123", "password!@#"])
def test_user_create_weak_password(weak_password, user_create_data):
    user_create_data["password"] = weak_password
    with pytest.raises(ValidationError):
        UserCreate(**user_create_data)

async def test_create_user_duplicate_username(async_client, verified_user):
    user_data = {
        "email": "new_email@example.com",
        "password": "AnotherPassword123!",
        "nickname": verified_user.nickname  # Duplicate username
    }
